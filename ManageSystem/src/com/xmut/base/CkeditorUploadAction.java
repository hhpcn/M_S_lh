...
public class CkeditorUploadAction  extends BaseAction{
	...
	private static final long serialVersionUID = -4229694803908453752L;
		...
		public String uploadImage() throws Exception {  
            ...
            String image_path = ServletActionContext.getServletContext().getInitParameter("UPLOAD_IMAGE_PATH");//从配置文件中读取要保存的位置
            String uploadPath = ServletActionContext.getServletContext().getRealPath(image_path+"/detailInfo/"+DateUtil.getDirDate());
           ...
           String URL=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort();
            ...
            // 返回“图像”选项卡并显示图片  
            out.println("<script type=\"text/javascript\">");    
            out.println("window.parent.CKEDITOR.tools.callFunction(" + callback + ",'" + URL+"/images/detailInfo/"+ DateUtil.getDirDate() +"/"+ fileName + "','')");    
            out.println("</script>");  
		...
		public String uploadGuideImg() throws Exception {  
            ...
            InputStream is = new FileInputStream(guideImg);  //浏览器端上传的文件
            String image_path = ServletActionContext.getServletContext().getInitParameter("UPLOAD_IMAGE_PATH");//从配置文件中读取要保存的位置
            String uploadPath = ServletActionContext.getServletContext().getRealPath(image_path+"/guide/"+DateUtil.getDirDate());
          ...
          // String URL=request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+ request.getContextPath()+"/";
           ...
           //数据库存的的是从web应用images下开始目录路径
           dataMap.put("guideImgUrl", "guide/"+ DateUtil.getDirDate() +"/"+ fileName);
		...
		public String removeFile() {
			...
			String image_path = ServletActionContext.getServletContext().getInitParameter("UPLOAD_IMAGE_PATH");//从配置文件中读取要保存的位置
            String uploadPath = ServletActionContext.getServletContext().getRealPath(image_path+"/"+message);
