var scripts = [null,"/ManageSystem/common/ace/dist/js/dropzone.min.js",
	               "/ManageSystem/common/ace/assets/js/date-time/moment.js",
	               "/ManageSystem/common/ace/assets/js/date-time/moment-with-locales.js",
	               "/ManageSystem/common/ace/assets/js/date-time/bootstrap-datetimepicker.js",
	               "/ManageSystem/common/ace/dist/js/jqGrid/jquery.jqGrid.min.js",
	               "/ManageSystem/common/ace/dist/js/jqGrid/i18n/grid.locale-cn.js",
	               "/ManageSystem/common/ace/assets/js/jquery.validate.js",null];
	...
	jQuery(function($) {
		...
		jQuery(grid_selector).jqGrid({
			...
			url:'/ManageSystem/productmanage/productAction_listProducts.action', //这是Action的请求地址 
			mtype: 'POST', 
			datatype: "json",
			height: 370,
			...
			editurl: "/ManageSystem/productmanage/productAction_edit.action",//nothing is saved
			caption: "产品表格"
		...
		try {
			  ...
			  var myDropzone = new Dropzone("#dropzone" , {
				...
				init:function(){
					...
					$.post("/ManageSystem/ckeditorUploadAction_removeFile.action",{message:message},function(data){
						//更新input guidemap的值
		...
		$("#saveProduct").on("click",function(){
			...
			var retImgArr = detailInfo.match(/src\s*=\s*[\"|\']?\s*[^>\"\'\s]*\.(jpg|jpeg|png|gif|bmp)[\"|\']?/gi);
			 if(retImgArr!=null){
				 ...
				 {
				    relativeImgArr=retImgArr[i].match(/\/detailInfo\s*[^>\"\'\s]*\.(jpg|jpeg|png|gif|bmp)/gi);
				    if(i>0)
					    imgUrls+="|";
			...
			$.ajax({
        		url:"/ManageSystem/productmanage/productAction_addProduct.action",
        		type:"post",
        		dataType:"json",
        		data:{
...
/*****************更新产品开始************************************************************/
		...
		$("#saveEditProduct").on("click",function(){
			...
			$.ajax({
        		url:"/ManageSystem/productmanage/productAction_editProduct.action",
        		type:"post",
        		dataType:"json",
        		data:{
        			    "product.id":productId,
...
function loadEditData(){
    //ajax，根据id到取得记录
	$.ajax({
		url:"/ManageSystem/productmanage/productAction_loadProductById.action",
		type:"post",
		dataType:"json",
		data:{
			"id":editStrs[0]
...
function viewEditGuideMap(product){
	...
	for(var i = 0;i<edit_guideImgArray.length;i++){
		...
		editDropzone.emit("thumbnail", mockFile, getImageUrl()+"/"+edit_guideImgArray[i]);
...
/*******************************公用的方法************************************/
	...
	function loadCategory(categorySelectId,kindSelectId){
		 //ajax获取栏目种类,初始化种类下拉选择框
		$.ajax({
    		url:"/ManageSystem/productmanage/categoryAction_listCategoryForKind.action",
    		type:"post",
    		dataType:"json",
    		success:function(data){
	...
	function loadKind(kindSelectId,currentOption){
		$.ajax({
    		url:"/ManageSystem/productmanage/kindAction_listKindsByCategoryId.action",
    		type:"post",
    		data:{
    			"id":currentOption
    		},
